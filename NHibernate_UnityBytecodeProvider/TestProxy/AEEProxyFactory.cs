...
namespace TestProxy
{
    public sealed class AEEProxyFactory
    {
        ...
        public AEEProxyFactory()
            : this(new AEEDefaultyProxyMethodBuilder()) { }

        public AEEProxyFactory(IProxyAssemblyBuilder proxyAssemblyBuilder)
            : this(new AEEDefaultyProxyMethodBuilder(), proxyAssemblyBuilder) { }

        public AEEProxyFactory(IProxyMethodBuilder proxyMethodBuilder)
            : this(new AEEDefaultyProxyMethodBuilder(), new DefaultProxyAssemblyBuilder()) { }

        public AEEProxyFactory(IProxyMethodBuilder proxyMethodBuilder, IProxyAssemblyBuilder proxyAssemblyBuilder)
        {
        ...
        public object CreateProxy(System.Type instanceType, IInterceptor interceptor, params System.Type[] baseInterfaces)
        {
            ...
            if (AEEObjectFactory.Contains(instanceType))
            {
                Type interceptedType = AEEObjectFactory.Resolve(instanceType).GetType();
                System.Type proxyType = CreateProxyType(interceptedType, baseInterfaces);
                AEEObjectFactory.RegisterTypeInterception(proxyType);
                result = AEEObjectFactory.Resolve(proxyType);
        ...
        private System.Type CreateUncachedProxyType(System.Type baseType, System.Type[] baseInterfaces)
        {
            ...
            // Implement IProxy
            var implementor = new AEEProxyImplementor();
